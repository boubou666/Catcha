...
<span class="bellwrap">
  <button class="small bell" onclick={toggle} aria-expanded={open} title={tr("Notifications")}>🔔</button>
...
{#if open}
  ...
  <div class="center panel" role="dialog" aria-label={tr("Notifications")}>
    <div class="row head">
      <h3 class="grow">{tr("Notifications")} <span class="muted">{filtering ? `${rows.length} of ${game.notices.length}` : game.notices.length}</span></h3>
      {#if game.unreadNotices > 0}<button class="small" onclick={() => game.markNoticesRead()}>{tr("Mark all read")}</button>{/if}
      {#if game.notices.length > 0}<button class="small" onclick={() => game.clearNotices()}>{tr("Clear")}</button>{/if}
      <button class="small" onclick={toggle} title={tr("Close")}>✕</button>
    </div>
    <div class="row">
      <input type="search" placeholder={tr("Search notifications…")} bind:value={filter.query} aria-label={tr("Search notifications")} />
      <select bind:value={filter.kind} aria-label={tr("Notification kind")}>
        <option value="any">{tr("All kinds")}</option>
        {#each NOTICE_KINDS as k}<option value={k}>{tr(NOTICE_KIND_LABEL[k])}</option>{/each}
      </select>
      <label class="chk"><input type="checkbox" bind:checked={filter.unreadOnly} /> {tr("Unread")}</label>
      {#if filtering}<button class="small" onclick={clear}>{tr("Reset")}</button>{/if}
    </div>
    <div class="list">
      {#if rows.length === 0}
      ...
      {#each rows as n (n.id)}
        <div class="notice {n.kind}" class:unread={!n.read}>
          ...
          <span class="grow">{n.msg ? tr(n.msg.key, n.msg.vars) : n.text}</span>
        </div>
      {/each}
    </div>
    <div class="prefs">
      <div class="muted small">{tr("Pop up as toasts (this device):")}</div>
      <div class="row">
        {#each NOTICE_KINDS as k}
          <label class="chk"><input type="checkbox" checked={game.toastPref[k]} onchange={(e) => game.setToastPref(k, e.currentTarget.checked)} /> {tr(NOTICE_KIND_LABEL[k])}</label>
        {/each}
      </div>
      <div class="muted tiny">{tr("Everything still lands here, up to the last")} {NOTICE_CAP}{tr(", for this session.")}</div>
    </div>
  </div>
{/if}
