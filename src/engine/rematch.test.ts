...
describe('Alpha rematches', () => {
  it('scale each win and go on a play-time cooldown', () => {
    ...
    expect(r).toMatchObject({ beaten: true, tier: 1, ready: false, level: 12, gold: 1000 });
    expect(r.secondsLeft).toBe(REMATCH_COOLDOWN_SEC);
    expect(r.hp).toBe(Math.round(wildHp(12) * chillet.hpMult * 1.25));
    ...
    expect(rematchInfo(s, chillet)).toMatchObject({ tier: 2, level: 13, gold: 1500, ready: false });
