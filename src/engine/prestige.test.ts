...
import { PRESTIGE_UPGRADES, BASE_ARK } from '../data/prestige';
...
describe('upgrades', () => {
  it('cost relics, cap at max level, and multiply the right stats', () => {
    ...
    expect(nextUpgradeCost(save, 'power')).toBe(2);
    expect(buyUpgrade(save, 'power')).toBe(true);
    expect(save.prestige.relics).toBe(98);
    expect(nextUpgradeCost(save, 'power')).toBe(4);
