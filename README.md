...
Box rows show the odds of catching another of that species (what condensing and breeding run on).
...
here?", `N` opens notifications, `/` or Ctrl+K focuses the global search, `S` saves, `?` shows the list.
Every single-key shortcut can be rebound or switched off under Settings → Keyboard (click the key, press a
new one; clashes are flagged, Reset restores the defaults); bindings live per device in `catcha.keys` as a
diff against the defaults (`src/engine/shortcuts.ts`, `src/state/keys.svelte.ts`). The digit, arrow, Space,
Ctrl+K and Esc shortcuts stay fixed.
...
Update overwrites with the current party; rename, delete; search by name or member species, filter by
readiness (everyone / some / nobody available) and sort by newest, name or most available. Stored in the
